...
Whilst developing / desigining, there are hinderances between developer-client interaction stemming from Technology Mismatch, Different Expectation, Design Process etc. 

Many a client who are in need of a Website App find themselves in a dilemma on who to hire, what tech to be used, and above all what the development process will entail. So the client would do what everyne does. Google. They would then realise there are a myriad of ways to solve their problems and approaching a skilled developer would be the best thing. So they search for portfolios  (by the way, the best designers/developers usually dont keep them as they are busy working) and eventually contact the best. 

After a series of e-mailing, they realise that the dude or dudess(I'm all for the affirmative action fellows :-) does only half of the work. The dude or dudess may only do [FrontEnd Design work](https://www.toptal.com/front-end/job-description) or may just do [BackEnd Development](https://www.toptal.com/back-end/job-description). They may be lucky to get a [FullStack Developer](https://www.rubysteps.com/articles/2014/so-you-want-to-become-a-full-stack-developer/), but getting one means that they must be prepared to fork some more depending on how urgent they need their work done and the specs of their product. 

Having been approached by many a person from a business point of view, I have come to realise that many are not aware what it means by what Client Side and Server Side means. They always say; I just want it to look "Amazing", "Beautiful!", "Great!", or even "Awesome!". This is what has led me to do this project. When tasked to do BackEnd design, many are astonished why the project doens't look nice. 

I have tried to create a snapshot of my designs using softwares to visualize the whole process of how data is stored and accessed via MVC. 

The project has both a [Client Side](), a [Server Side]() and the full fledge  [Website App]() with both Technologies merged together. 

I used SoC in order to create an undersanding of the different functionalities and what Designers (FrontEnd), Developers(BackEnd) and a mixture of both (FullStack) undergo to realise your ideas into a design. By the way, I think FullStackers deserve a name of their own. eg. Desvelopers? 

Anyway, the FrontEnd portion runs on Bootstrap 3 whilst the BackEnd system runs on Ruby on Rails.
I refrained to many gems like Simple Forms, that make coding easier, and tried to write the code just as an estimate on the time to be spent on a particular project. Another reason is because it helps me come with ideas. Oh yes! Programming makes you  think. 
...
I used this gem to pupulate my Database with data. It's as handy as Lorem Ipsum is handy in generating blind text. 
...
To crack an hash algorithm one hte hacker would need to find a collision within the hash using a preimage attack. 
...
By the way, that is one of my real passwords in gmail with the real password that I used in the examples and would dare anyone to go ahead and crack the password. I've even provided the hash algorithm so no primary hacking to get to the hash.
